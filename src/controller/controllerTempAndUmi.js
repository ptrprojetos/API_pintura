...
const getTempAndUmi03 = async (req, res) => {
  try {
    ...
    if(sensor03.mensage === 'Não foi possível obter os dados em 10 segundos')
      return res.status(400).json({message: 'erro ao conectar com sensores'})
    else return res.status(200).json(sensor03);
  } catch (error) {
    console.log(error);
    res.status(500).json({
...
const getTempAndUmi02 = async (req, res) => {
  try {
    ...
    if(sensor02.mensage === 'Não foi possível obter os dados em 10 segundos')
      return res.status(400).json({message: 'erro ao conectar com sensores'})
    else return res.status(200).json(sensor02);
  } catch (error) {
    console.log(error);
    res.status(500).json({
      error: 'Erro interno do servidor',
